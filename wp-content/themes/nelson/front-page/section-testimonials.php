<div class="front_page_section front_page_section_testimonials<?php
	$nelson_scheme = nelson_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $nelson_scheme ) && ! nelson_is_inherit( $nelson_scheme ) ) {
		echo ' scheme_' . esc_attr( $nelson_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( nelson_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( nelson_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$nelson_css      = '';
		$nelson_bg_image = nelson_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$nelson_anchor_icon = nelson_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$nelson_anchor_text = nelson_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( nelson_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$nelson_css      = '';
			$nelson_bg_mask  = nelson_get_theme_option( 'front_page_testimonials_bg_mask' );
			$nelson_bg_color_type = nelson_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $nelson_bg_color_type ) {
				$nelson_bg_color = nelson_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$nelson_caption = nelson_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $nelson_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $nelson_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $nelson_caption, 'nelson_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$nelson_description = nelson_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $nelson_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $nelson_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $nelson_description ), 'nelson_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
				...
				if ( ! nelson_exists_trx_addons() ) {
					...
					nelson_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
				}
			}
			?>
			</div>
