...
if ( ! function_exists( 'nelson_need_page_title' ) ) {
	function nelson_need_page_title() {
		return ! is_front_page() && apply_filters( 'nelson_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'nelson_show_layout' ) ) {
	function nelson_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'nelson_get_logo_image' ) ) {
	function nelson_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = nelson_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = nelson_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
		}
		$logo_retina = nelson_is_on( nelson_get_theme_option( 'logo_retina_enabled' ) )
						? nelson_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? nelson_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? nelson_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

// Return header video (if set)
if ( ! function_exists( 'nelson_get_header_video' ) ) {
	function nelson_get_header_video() {
		$video = '';
		if ( apply_filters( 'nelson_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( nelson_check_theme_option( 'header_video' ) ) {
				$video = nelson_get_theme_option( 'header_video' );
...
// Show post banner
if ( ! function_exists( 'nelson_show_post_banner' ) ) {
	function nelson_show_post_banner( $banner_pos = '') {
		if ( is_singular( 'post' ) && '' !== $banner_pos ){
			$banner_code   = nelson_get_theme_option( $banner_pos . '_banner_code' );
			$banner_img    = nelson_get_theme_option( $banner_pos . '_banner_img' );
			$banner_height = nelson_get_theme_option( $banner_pos . '_banner_height' );
			$banner_class  = !empty( $banner_img )
								? ( 'background' == $banner_pos ? '' : 'banner_with_image ' )
									. nelson_add_inline_css_class( 
										'background-image:url(' . esc_url( $banner_img ) . ');'
										. ( ! empty( $banner_height )
											? 'min-height:' . nelson_prepare_css_value( $banner_height ) . ';'
											: ''
											)
										)
								: '';
			$banner_link   = nelson_get_theme_option( $banner_pos . '_banner_link' );
			if ( ! empty( $banner_code ) || ! empty( $banner_img ) ) {
				...
				nelson_show_layout( wp_kses( $banner_code ), 'nelson_kses_content' );
...
if ( ! function_exists( 'nelson_show_post_featured_image' ) ) {
	function nelson_show_post_featured_image( $thumb_bg = false ) {
		// Featured image
		if ( ! nelson_sc_layouts_showed( 'featured' ) && strpos( nelson_get_post_content(), '[trx_widget_banner]' ) === false ) {
			do_action( 'nelson_action_before_post_featured' );
			...
			do_action( 'nelson_action_after_post_featured' );
		} elseif ( nelson_is_on( nelson_get_theme_option( 'seo_snippets' ) ) && has_post_thumbnail() ) {
			?>
...
if ( ! function_exists( 'nelson_show_post_title_and_meta' ) ) {
	function nelson_show_post_title_and_meta( $need_content_wrap = false ) {

		// Title and post meta
		if ( ( ! nelson_sc_layouts_showed( 'title' ) || ! nelson_sc_layouts_showed( 'postmeta' ) ) ) {
			...
			<div class="post_header post_header_single entry-header">
				...
				// Post meta
				$meta_components = nelson_array_get_keys_by_value( nelson_get_theme_option( 'meta_parts' ) );
				// Post title
				$seo = nelson_is_on( nelson_get_theme_option( 'seo_snippets' ) );
				if ( ! nelson_sc_layouts_showed( 'title' ) ) {
					the_title( '<h1 class="post_title entry-title"' . ( $seo ? ' itemprop="headline"' : '' ) . '>', '</h1>' );
				}
				// Post subtitle
				$post_subtitle = nelson_get_theme_option( 'post_subtitle' );
				if ( ! empty( $post_subtitle ) ) {
					?>
					<div class="post_subtitle">
						<?php nelson_show_layout( $post_subtitle ); ?>
					</div>
					<?php
				}
				// Post meta
				if ( ! nelson_sc_layouts_showed( 'postmeta' ) && nelson_is_on( nelson_get_theme_option( 'show_post_meta' ) ) ) {
					nelson_show_post_meta(
						apply_filters(
							'nelson_filter_post_meta_args',
							array(
...
if ( ! function_exists( 'nelson_show_post_content' ) ) {
	function nelson_show_post_content( $args = array(), $otag='', $ctag='' ) {
		$plain = true;
		...
		if ( has_excerpt() ) {
			...
			nelson_show_layout( nelson_filter_post_content( get_the_content('') ) );
			...
			$quote = nelson_get_tag( nelson_filter_post_content( get_the_content() ), '<blockquote', '</blockquote>' );
			if ( ! empty( $quote ) ) {
				nelson_show_layout( wpautop( $quote ) );
				$plain = false;
			} else {
				nelson_show_layout( nelson_filter_post_content( get_the_content() ) );
			}
			...
			if ( $plain ) {
				$len = ! empty( $args['hide_excerpt'] )
							...
							: ( ! empty( $args['excerpt_length'] )
								...
								: nelson_get_theme_option( 'excerpt_length' )
								);
				$output = nelson_excerpt( $output, $len );
			}
		}
		nelson_show_layout( $output, $otag, $ctag);
	}
}


// Show post link 'Read more' in the blog posts
if ( ! function_exists( 'nelson_show_post_more_link' ) ) {
	function nelson_show_post_more_link( $args = array(), $otag='', $ctag='' ) {
		nelson_show_layout(
			'<a class="more-link" href="' . esc_url( get_permalink() ) . '">'
				. ( ! empty( $args['more_text'] )
						? esc_html( $args['more_text'] )
						: esc_html__( 'Read more', 'nelson' )
						)
			. '</a>',
			$otag,
			$ctag
...
if ( ! function_exists( 'nelson_show_post_meta' ) ) {
	function nelson_show_post_meta( $args = array() ) {
		if ( is_single() && nelson_is_off( nelson_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta<?php echo ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : ''; ?>">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					// Post categories
					$cats = get_post_type() == 'post' ? get_the_category_list( ', ' ) : apply_filters( 'nelson_filter_get_post_categories', '' );
					if ( ! empty( $cats ) ) {
						nelson_show_layout( $cats, '<span class="post_meta_item post_categories">', '</span> ');
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'nelson_filter_get_post_date', nelson_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '' ) );
					if ( ! empty( $dt ) ) {
						nelson_show_layout(
							$dt,
					...
					if ( $author_id > 0 ) {
						...
						$author_avatar = get_avatar( get_the_author_meta( 'user_email', $author_id ), 32 * nelson_get_retina_multiplier() );
						echo '<a class="post_meta_item post_author" rel="author" href="' . esc_url( $author_link ) . '">'
								. ( nelson_get_theme_setting( 'show_author_avatar' )
					...
					if ( !is_singular() || have_comments() || comments_open() ) {
						...
						echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-light">'
								...
								. '<span class="post_meta_label">' . (1==$post_comments ? esc_html__('Comment', 'nelson') : esc_html__('Comments', 'nelson')) . '</span>'
							. '</a> ';
					}
					...
					if ( function_exists( 'trx_addons_get_post_views' ) ) {
						...
						echo ( is_singular()
							...
							. ' class="post_meta_item post_meta_views trx_addons_icon-eye">'
								...
								. '<span class="post_meta_label">' . (1==$post_views ? esc_html__('View', 'nelson') : esc_html__('Views', 'nelson')) . '</span>'
							. ( is_singular()
								? '</span> '
								: '</a> '
								);
				...
				// Likes (Emotions)
					...
					if ( function_exists( 'trx_addons_get_post_likes' ) ) {
						...
						$liked = isset( $_COOKIE['trx_addons_likes'] ) ? nelson_get_value_gpc('trx_addons_likes') : '';
						...
						echo ( true == $emotions_allowed
								...
								: '<a href="#"'
									...
									. ' title="' . ( ! empty( $allow ) ? esc_attr__( 'Like', 'nelson') : esc_attr__( 'Dislike', 'nelson' ) ) . '"'
									. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
									. ' data-likes="' . esc_attr( $post_likes ) . '"'
									. ' data-title-like="' . esc_attr__( 'Like', 'nelson') . '"'
									. ' data-title-dislike="' . esc_attr__( 'Dislike', 'nelson' ) . '"'
									. '>'
								)
									. '<span class="post_meta_number">' . esc_html($post_likes) . '</span>'
									. '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? (1==$post_likes ? esc_html__('Reaction', 'nelson') : esc_html__('Reactions', 'nelson'))
											: (1==$post_likes ? esc_html__('Like', 'nelson') : esc_html__('Likes', 'nelson'))
											)
									. '</span>'
								. '</a> ';
					}
					...
					nelson_show_share_links(
						array(
							...
							'caption' => 'drop' == $args['share_type'] ? esc_html__( 'Share', 'nelson' ) : '',
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'nelson' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

				} else {
					// Custom counter
					do_action( 'nelson_action_show_post_meta', $comp, get_the_ID(), $args );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'nelson_show_post_featured' ) ) {
	function nelson_show_post_featured( $args = array() ) {

		$args = array_merge(
			array(
				'popup'         => nelson_get_theme_option( 'video_in_popup' ), // Open video in popup
				'hover'         => nelson_get_theme_option( 'image_hover' ),    // Hover effect
				...
				'show_no_image' => nelson_is_on( nelson_get_theme_setting( 'allow_no_image' ) ),  // Display 'no-image.jpg' if post haven't thumbnail
				'seo'           => nelson_is_on( nelson_get_theme_option( 'seo_snippets' ) ),     // Add SEO-snippets
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: nelson_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? nelson_get_no_image( '', true ) : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = nelson_get_post_image();
				if ( ! empty( $image ) ) {
					$image = nelson_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . nelson_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						nelson_is_on( nelson_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( nelson_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				$output = '<div class="post_featured' . ( $args['class'] ? ' ' . esc_attr( $args['class'] ) : '' ) . '"'
					. ( $args['seo'] ? ' itemscope="itemscope" itemprop="image" itemtype="' . esc_attr( nelson_get_protocol( true ) ) . '//schema.org/ImageObject"' : '')
				...
				$output .= '>';
				nelson_show_layout( $output );
				if ( has_post_thumbnail() && $args['seo'] ) {
					$nelson_attr = nelson_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $nelson_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $nelson_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb
				...
				|| ( ! $args['thumb_only']
						&& ( in_array( $post_format, array( 'image', 'audio', 'video' ) )
							|| ( 'gallery' == $post_format && nelson_exists_trx_addons() )
							)
					)
			) {
				...
				if ( $args['thumb_bg'] ) {
					...
					$output = nelson_get_slider_layout( $slider_args );
					if ( '' != $output ) {
						nelson_show_layout( $output );
					}
					//------- End: Moved down --------
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) || ! empty( $args['video'] ) ) ) {
					$post_content = nelson_get_post_content();
					$post_content_parsed = $post_content;

					if ( 'video' == $post_format || ! empty( $args['video'] ) ) {
						$video = ! empty( $args['video'] ) ? $args['video'] : nelson_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = nelson_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = nelson_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = nelson_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							...
							if ( $has_thumb && ! empty( $args['popup'] ) && function_exists( 'trx_addons_get_video_layout' ) ) {
								...
								if ( ! empty( $popup[0] ) && ! empty( $popup[1] ) ) {
									if ( preg_match( '/<a .*<\/a>/', $popup[0], $matches ) && ! empty( $matches[0] ) ) {
										$video_out = true;
										?>
										<div class="post_video_hover post_video_hover_popup"><?php nelson_show_layout( $matches[0] ); ?></div>
										<?php
										nelson_show_layout($popup[1]);
									}
								}
							}
							if ( ! $video_out ) {
								if ( $has_thumb ) {
									$video = nelson_make_video_autoplay( $video );
									?>
						...
						$audio = nelson_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = nelson_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = nelson_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								if ( strpos( $audio, '<audio' ) !== false ) {
									$media_author = nelson_get_tag_attrib( $audio, '<audio>', 'data-author' );
									$media_title  = nelson_get_tag_attrib( $audio, '<audio>', 'data-caption' );
								}
								if ( empty( $media_author) &&  empty( $media_title) ) {
									$media = urldecode( nelson_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
									if ( ! empty( $media ) ) {
										...
										// Parse tag params
										$media_author = strip_tags( nelson_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
										$media_title  = strip_tags( nelson_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );

									}
								}
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php nelson_show_layout( $media_author ); ?></div>
									<?php
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php nelson_show_layout( $media_title ); ?></h5>
									<?php
								}
								// Display audio
								nelson_show_layout( $audio );
								?>
							</div>
							<?php
						}
...
if ( ! function_exists( 'nelson_get_no_image' ) ) {
	function nelson_get_no_image( $no_image = '', $need = false ) {
		static $no_image_url = '';
		$img = nelson_get_theme_option( 'no_image' );
		if ( empty( $img ) && ( $need || nelson_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( empty( $no_image_url ) ) {
				$no_image_url = nelson_get_file_url( 'images/no-image.jpg' );
			}
			$img = $no_image_url;
		}
		if ( ! empty( $img ) ) {
...
if ( ! function_exists( 'nelson_add_bg_in_post_nav' ) ) {
	function nelson_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), nelson_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), nelson_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		wp_add_inline_style( 'nelson-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'nelson_show_related_posts' ) ) {
	function nelson_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : nelson_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) nelson_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = nelson_get_theme_option( 'related_position' );
			$related_style = nelson_get_theme_option( 'related_style' );
			...
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				<<?php echo esc_html( $related_tag ); ?> class="section_title related_wrap_title"><?php
					if ( ! empty( $title ) ) {
						...
						esc_html_e( 'You May Also Like', 'nelson' );
					}
				...
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = nelson_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = nelson_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = nelson_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'nelson_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					nelson_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						if ( ! apply_filters( 'nelson_filter_related_post_showed', false, $args, $style ) ) {
							get_template_part( apply_filters( 'nelson_filter_get_template_part', 'templates/related-posts', $style ), $style );
						}
						?></div><?php
					}
				?></div><?php						if ( $args['slider'] ) {
					nelson_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'nelson_show_related_posts_callback' ) ) {
	add_action( 'nelson_action_related_posts', 'nelson_show_related_posts_callback' );
	function nelson_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'nelson_filter_show_related_posts', false ) ) {
			$nelson_related_posts   = (int) nelson_get_theme_option( 'related_posts' );
			$nelson_related_columns = (int) nelson_get_theme_option( 'related_columns' );
			$nelson_related_style   = nelson_get_theme_option( 'related_style' );
			if ( (int) nelson_get_theme_option( 'show_related_posts' ) && $nelson_related_posts > 0 ) {
				nelson_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $nelson_related_posts ) ),
						'columns'        => max( 1, min( 6, $nelson_related_posts, $nelson_related_columns ) ),
					),
					$nelson_related_style
				);
			}
		}
	}
...
if ( ! function_exists( 'nelson_show_portfolio_posts' ) ) {
	function nelson_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? nelson_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = nelson_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = nelson_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$nelson_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the nelson_ajax_get_posts action
if ( ! function_exists( 'nelson_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_nelson_ajax_get_posts', 'nelson_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_nelson_ajax_get_posts', 'nelson_ajax_get_posts_callback' );
	function nelson_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'nelson' );
		}
...
// Show pagination
if ( ! function_exists( 'nelson_show_pagination' ) ) {
	function nelson_show_pagination( $args = array() ) {
		global $wp_query;
		$pagination = ! empty( $args[ 'pagination' ] )
						? $args[ 'pagination' ]
						: nelson_get_theme_option( 'blog_pagination' );
		$prefix     = ! empty( $args[ 'class_prefix' ] )
						? $args[ 'class_prefix' ]
						: 'nav';
		if ( 'pages' == $pagination ) {
			nelson_show_layout( str_replace( "\n", '', get_the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'nelson' ),
					'next_text'          => esc_html__( '>', 'nelson' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'nelson' ) . ' </span>',
				)
			) ) );
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="<?php echo esc_attr( $prefix ); ?>-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load more posts', 'nelson' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="<?php echo esc_attr( $prefix ); ?>-links-old">
				<span class="<?php echo esc_attr( $prefix ); ?>-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'nelson' ) : esc_html__( 'Newest posts', 'nelson' ) ); ?></span>
				<span class="<?php echo esc_attr( $prefix ); ?>-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'nelson' ) : esc_html__( 'Older posts', 'nelson' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'nelson_single_comments_field' ) ) {
	function nelson_single_comments_field( $args ) {
		...
		if ( 'text' == $args['field_type'] ) {
			...
			$html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
					. ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'nelson_kses_content' ) . '</label>';
		} else {
		...
		if ( 'default' != $args['form_style'] && in_array( $args['field_type'], array( 'text', 'textarea' ) ) ) {
			$html .= '<span class="sc_form_field_hover">'
						...
						. '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'nelson_kses_content' ) . '</span>'
					. '</span>';
		}
		$html .= '</span></div>';
		return $html;
