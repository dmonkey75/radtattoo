...
if ( ! function_exists( 'nelson_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'nelson_hovers_theme_setup3', 3 );
	function nelson_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		nelson_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'nelson' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'nelson' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'nelson' ),
						'slide_left'   => esc_html__( 'Slide from Left', 'nelson' ),
						'slide_right'  => esc_html__( 'Slide from Right', 'nelson' ),
						'slide_top'    => esc_html__( 'Slide from Top', 'nelson' ),
						'slide_bottom' => esc_html__( 'Slide from Bottom', 'nelson' ),
								),
					'type'    => 'hidden',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'nelson' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'nelson' ) ),
					'std'      => 'icon',
					'override' => array(
						'mode'    => 'icon',
						'section' => esc_html__( 'Content', 'nelson' ),
					),
					'options'  => nelson_get_list_hovers(),
					'type'     => 'hidden',
				),
			)
		);
...
if ( ! function_exists( 'nelson_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'nelson_hovers_theme_setup9', 9 );
	function nelson_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'nelson_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'nelson_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'nelson_hovers_responsive_styles', 2000 );
		add_filter( 'nelson_filter_localize_script', 'nelson_hovers_localize_script' );
		add_filter( 'nelson_filter_merge_scripts', 'nelson_hovers_merge_scripts' );
		add_filter( 'nelson_filter_merge_styles', 'nelson_hovers_merge_styles' );
		add_filter( 'nelson_filter_merge_styles_responsive', 'nelson_hovers_merge_styles_responsive' );
		add_filter( 'nelson_filter_get_css', 'nelson_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'nelson_hovers_frontend_scripts' ) ) {
		function nelson_hovers_frontend_scripts() {
		if ( nelson_is_on( nelson_get_theme_option( 'debug_mode' ) ) ) {
			$nelson_url = nelson_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $nelson_url ) {
				wp_enqueue_script( 'nelson-hovers', $nelson_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'nelson_hovers_frontend_styles' ) ) {
		function nelson_hovers_frontend_styles() {
		if ( nelson_is_on( nelson_get_theme_option( 'debug_mode' ) ) ) {
			$nelson_url = nelson_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $nelson_url ) {
				wp_enqueue_style( 'nelson-hovers', $nelson_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'nelson_hovers_responsive_styles' ) ) {
		function nelson_hovers_responsive_styles() {
		if ( nelson_is_on( nelson_get_theme_option( 'debug_mode' ) ) ) {
			$nelson_url = nelson_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $nelson_url ) {
				wp_enqueue_style( 'nelson-hovers-responsive', $nelson_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'nelson_hovers_merge_styles' ) ) {
		function nelson_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'nelson_hovers_merge_styles_responsive' ) ) {
		function nelson_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'nelson_hovers_localize_script' ) ) {
		function nelson_hovers_localize_script( $arr ) {
		$arr['button_hover'] = nelson_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'nelson_hovers_merge_scripts' ) ) {
		function nelson_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'nelson_hovers_add_icons' ) ) {
	function nelson_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php nelson_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'nelson' ) : esc_html__( 'Buy now', 'nelson' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'nelson' );
					} else {
						esc_html_e( 'Details', 'nelson' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php nelson_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php nelson_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php nelson_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php nelson_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'nelson_hovers_get_css' ) ) {
		function nelson_hovers_get_css( $css, $args ) {
