...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'nelson_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'nelson_trx_addons_theme_specific_components' );
	function nelson_trx_addons_theme_specific_components( $enable = false ) {
		return NELSON_THEME_FREE
					? false     // Free version
					: false;     // Pro version or Developer mode
	}
}

if ( ! function_exists( 'nelson_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'nelson_trx_addons_theme_specific_setup1', 1 );
	function nelson_trx_addons_theme_specific_setup1() {
		if ( nelson_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'nelson_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'nelson_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'nelson_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'nelson_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'nelson_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'nelson_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'nelson_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'nelson_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'nelson_trx_addons_theme_specific_setup' );
	function nelson_trx_addons_theme_specific_setup() {
		if ( nelson_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'nelson_action_before_post_meta', 'nelson_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'nelson_trx_addons_cv_enable' ) ) {
		function nelson_trx_addons_cv_enable( $enable = false ) {
...
// Demo mode
if ( ! function_exists( 'nelson_trx_addons_demo_enable' ) ) {
		function nelson_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'nelson_trx_addons_edd_themes_market_enable' ) ) {
		function nelson_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'nelson_trx_addons_api_list' ) ) {
		function nelson_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'nelson_trx_addons_cpt_list' ) ) {
		function nelson_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'nelson_trx_addons_sc_list' ) ) {
		function nelson_trx_addons_sc_list( $list = array() ) {
        ...
        $list['blogger']['templates']['lay_portfolio']['style-1'] = array(
            'title'  => esc_html__('Style 1', 'nelson'),
            'layout' => array(
                'featured' => array(
                ),
                'content' => array(
        ...
        $list['blogger']['templates']['product']['classic-product'] = array(
            'title'  => esc_html__('classic (for product)', 'nelson'),
            'layout' => array(
                'featured' => array(
                ),
                'content' => array(
        ...
        $list['blogger']['templates']['list']['meta_classic'] = array(
            'title' => esc_html__('List classic (meta)', 'nelson'),
            'layout' => array(
                'content' => array(
                    'title', 'meta', 'readmore'
                )
        ...
        $list['blogger']['templates']['default']['simple_on_cover'] = array(
            'title' => esc_html__('Simple on bg', 'nelson'),
            'layout' => array(
                'featured' => array(
                ),
                'content' => array(
...
// Widgets
if ( ! function_exists( 'nelson_trx_addons_widgets_list' ) ) {
		function nelson_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'nelson_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'nelson_trx_addons_menu_cache' );
	function nelson_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'nelson_trx_addons_localize_script' ) ) {
	...
	function nelson_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = nelson_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'nelson_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'nelson_trx_addons_sc_type', 10, 2 );
	function nelson_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = nelson_array_merge( $list, nelson_get_list_blog_styles( false, 'sc' ) );
		}
        if($sc == 'trx_sc_socials') {
        ...
        if ( 'trx_sc_blogger' == $sc ) {
            ...
            $list['lay_portfolio'] = esc_html__('Layout Portfolio', 'nelson' );
        }
...
if ( ! function_exists( 'nelson_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'nelson_trx_addons_sc_prepare_atts', 10, 2 );
	function nelson_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = nelson_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = nelson_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'nelson_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'nelson_trx_addons_sc_atts', 10, 2 );
	function nelson_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'nelson_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'nelson_trx_addons_sc_map', 10, 2 );
	function nelson_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'nelson' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'nelson' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( nelson_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'nelson' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'nelson' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( nelson_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'nelson_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'nelson_trx_addons_sc_output', 10, 4 );
	function nelson_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! nelson_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
		...
		} elseif ( in_array( $sc, array( 'trx_sc_osmap' ) ) ) {
			if ( ! empty( $atts['scheme'] ) && ! nelson_is_inherit( $atts['scheme'] ) ) {
				$output = str_replace( 'class="sc_osmap_content', 'class="sc_osmap_content scheme_' . esc_attr( $atts['scheme'] ), $output );
			}
			if ( ! empty( $atts['color_style'] ) && ! nelson_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
				$output = str_replace( 'class="sc_osmap_content ', 'class="sc_osmap_content color_style_' . esc_attr( $atts['color_style'] ) . ' ', $output );
			}
...
if ( ! function_exists( 'nelson_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'nelson_trx_addons_sc_item_link_classes', 10, 3 );
	function nelson_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! nelson_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'nelson_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'nelson_trx_addons_sc_item_title_tag' );
	function nelson_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'nelson_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'nelson_trx_addons_sc_item_button_args', 10, 3 );
	function nelson_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'nelson_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'nelson_trx_addons_slider_title', 10, 2 );
	function nelson_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'nelson_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'nelson_trx_addons_sc_googlemap_styles' );
	function nelson_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'nelson' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'nelson_trx_addons_action_before_post_meta' ) ) {
		function nelson_trx_addons_action_before_post_meta() {
		if ( nelson_exists_trx_addons() ) {
...
if ( ! function_exists( 'nelson_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'nelson_trx_addons_tiny_mce_style_formats' );
	function nelson_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'nelson' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'nelson' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'nelson_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'nelson_filter_allow_override_header_image', 'nelson_trx_addons_allow_override_header_image' );
	function nelson_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					nelson_is_team_page()
					|| nelson_is_cars_page()
					|| nelson_is_cars_agents_page()
					|| nelson_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'nelson_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'nelson_trx_addons_thumb_size', 10, 2 );
	function nelson_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
...
if ( ! function_exists( 'nelson_trx_addons_override_options_fields' ) ) {
	add_filter( 'trx_addons_filter_override_options_fields', 'nelson_trx_addons_override_options_fields', 10, 2 );
	function nelson_trx_addons_override_options_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'nelson' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'nelson' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
...
if ( ! function_exists( 'nelson_filter_get_list_menu_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_menu_hover', 'nelson_filter_get_list_menu_hover' );
    function nelson_filter_get_list_menu_hover( $list ) {
        unset( $list['fade_box'] );
...
if ( ! function_exists( 'nelson_filter_get_list_input_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_input_hover', 'nelson_filter_get_list_input_hover' );
    function nelson_filter_get_list_input_hover( $list ) {
        unset( $list['accent'] );
        unset( $list['path'] );
        unset( $list['jump'] );
...
if ( ! function_exists( 'nelson_cpt_get_list_options' ) ) {
    add_filter( 'trx_addons_cpt_list_options', 'nelson_cpt_get_list_options' );
    function nelson_cpt_get_list_options( $args ) {
        $args['services_form']['type'] = 'hidden';
        $args['services_form']['std'] = 'none';
        $args['team_form']['type'] = 'hidden';
        $args['team_form']['std'] = 'none';
        return $args;
    }
}