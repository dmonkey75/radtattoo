...
( function( $ ) {
	...
	var $style = $( '#nelson-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		var style_container = '<' + 'style type="text/css" id="nelson-customizer-inline-css" />',
			$theme_css      = $( '#nelson-custom-inline-css' );
		if ($theme_css.length == 0) {
			$theme_css = $( '#nelson-custom-css' );
		}
		if ($theme_css.length == 0) {
		...
		$style = $( '#nelson-customizer-inline-css' );
	}
			...
			api.preview.bind(
				'refresh-other-controls', function( obj ) {
					...
					if (id.indexOf( 'body_style' ) == 0) {
						...
						$( '.sc_layouts_logo .logo_text' ).html( nelson_prepare_macros( val ) );

					} else if (id == 'blogdescription') {
						$( '.sc_layouts_logo .logo_slogan' ).html( nelson_prepare_macros( val ) );

					} else if (id == 'copyright') {
						$( '.copyright_text' ).html( nelson_prepare_macros( val ) );
						...
						if (id == 'front_page_bg_image') {
							...
							if (color_type == 'none') {
								...
								color = typeof nelson_color_schemes[scheme] != 'undefined' ? nelson_color_schemes[scheme]['colors']['bg_color'] : '';
							}
							$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : nelson_hex2rgba( color, mask ) ) );
							...
							.height( 'auto' )
							.toggleClass( 'nelson-full-height sc_layouts_flex sc_layouts_columns_middle', val );
							$( window ).trigger( 'resize' );
						}
					}
				}
