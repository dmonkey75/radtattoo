/* global nelson_color_schemes, nelson_dependencies, Color */
...
( function( api ) {
	...
	for (var i in nelson_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'nelson-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'nelson-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'nelson-vars' );
	...
	api.bind(
		'ready', function() {
			...
			// Add 'reset' button
			jQuery( '#customize-header-actions #save' ).before( '<input type="button" class="button customize-action-reset" value="' + nelson_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' ).on(
				'click', function(e) {
					if (confirm( nelson_customizer_vars['msg_reset_confirm'] )) {
			...
			if (nelson_customizer_vars && nelson_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( nelson_customizer_vars['theme_name_suffix'] );
			}

			// Blur the "load fonts" fields - regenerate options lists in the font-family controls
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' ).on( 'focusout', nelson_customizer_update_load_fonts );

			// Click on the actions button
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' ).on( 'click', nelson_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' ).each(
				function () {
					nelson_customizer_check_dependencies( jQuery( this ) );
				}
			);
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' ).on(
				'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							nelson_customizer_check_dependencies( section );
						}
					}
				}
			);
	...
	api.bind(
		'change', function(obj) {
			...
			nelson_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
			nelson_customizer_refresh_preview( obj );
		}
	);
	...
	api.bind(
		'refresh_schemes', function() {
			...
			for (var i in cssTemplate) {
				...
				i = i.replace( 'scheme_', '' );
				if (typeof nelson_color_schemes[i] === 'undefined') {
					delete cssTemplate['scheme_' + i];
					jQuery( '#tmpl-nelson-color-scheme-' + i ).remove();
				} else if (tpl_idx == '') {
					tpl_idx     = i;
					tpl_content = jQuery( '#tmpl-nelson-color-scheme-' + i ).html().trim();
				}
			}
			// Add new templates
			...
			for (var i in nelson_color_schemes) {
				if (typeof cssTemplate['scheme_' + i] === 'undefined') {
					jQuery( '#tmpl-nelson-color-scheme-' + tpl_idx )
					.clone()
					.attr( 'id', 'tmpl-nelson-color-scheme-' + i )
					.html( tpl_content.replace( regex, '.scheme_' + i ) )
					.insertAfter( jQuery( '#tmpl-nelson-color-scheme-' + tpl_idx ) );
					cssTemplate['scheme_' + i] = wp.template( 'nelson-color-scheme-' + i );
				}
			}
		}
	...
	for (var action in nelson_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (nelson_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = nelson_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = nelson_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function nelson_customizer_get_field_value(fld) {
	...
	function nelson_customizer_check_dependencies(cont) {
		cont.find( '.customize-control' ).each(
			function() {
				...
				var fld    = null, val = '', i;
				var depend = false;
				for (fld in nelson_dependencies) {
					if (fld == id) {
						depend = nelson_dependencies[id];
						break;
					}
				}
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = nelson_customizer_get_field_value( fld );
					...
					if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
						ctrl.show().removeClass( 'nelson_options_no_use' );
					} else {
						ctrl.hide().addClass( 'nelson_options_no_use' );
					}
				}
	...
	function nelson_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( nelson_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					nelson_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in nelson_theme_fonts) {
					for (rule in nelson_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							nelson_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function nelson_customizer_actions(e) {
	...
	function nelson_customizer_update_theme_vars(opt, value) {
		nelson_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function nelson_customizer_update_theme_fonts(opt, rule, value) {
		nelson_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function nelson_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= nelson_customizer_vars['max_load_fonts']; i++) {
	...
	function nelson_customizer_update_css() {
		...
		var vars = nelson_customizer_add_theme_vars( nelson_theme_vars );

		// Make styles and add its to the css
		css += nelson_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = nelson_customizer_add_theme_fonts( nelson_theme_fonts );

		// Make styles and add its to the css
		css += nelson_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in nelson_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( nelson_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in nelson_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in nelson_color_schemes[scheme].colors) {
				colors[i] = nelson_color_schemes[scheme].colors[i];
			}
			...
			var tmpl = jQuery( '#tmpl-nelson-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function nelson_customizer_add_theme_colors(colors) {
		if (nelson_additional_colors) {
			var clr = '', v = '';
			for (var k in nelson_additional_colors) {
				v   = nelson_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + nelson_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + nelson_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + nelson_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function nelson_customizer_add_theme_vars(vars) {
		...
		for (var i in vars) {
			if (i == 'rad') {
				...
				rez['rad']   = nelson_customizer_prepare_css_value( vars['rad'] );
				...
				rez['page']              = nelson_customizer_prepare_css_value( vars['page'] );
				rez['page_boxed']        = nelson_customizer_prepare_css_value( vars['page'] + 2 * vars['page_boxed_extra'] );
				rez['sidebar']           = nelson_customizer_prepare_css_value( vars['sidebar'] );
				rez['gap']               = nelson_customizer_prepare_css_value( vars['gap'] );
				rez['content']           = nelson_customizer_prepare_css_value( vars['page'] - vars['sidebar'] - vars['gap'] );
				rez['sidebar_gap']       = nelson_customizer_prepare_css_value( vars['sidebar'] + vars['gap'] );
				rez['sidebar_prc']       = vars['sidebar'] / vars['page'];
				rez['gap_prc']           = vars['gap'] / vars['page'];
				rez['page_1_1']          = nelson_customizer_prepare_css_value( vars['page'] );
				rez['page_1_2']          = nelson_customizer_prepare_css_value( vars['page'] / 2 );
				rez['page_1_3']          = nelson_customizer_prepare_css_value( vars['page'] / 3 );
				rez['page_2_3']          = nelson_customizer_prepare_css_value( vars['page'] / 3 * 2 );
				rez['page_1_4']          = nelson_customizer_prepare_css_value( vars['page'] / 4 );
				rez['page_3_4']          = nelson_customizer_prepare_css_value( vars['page'] / 4 * 3 );
				var margins              = [10,20,30,40,60];
				for (var m = 0; m < margins.length; m++) {
					rez['page' + margins[m]]    = nelson_customizer_prepare_css_value( vars['page'] + margins[m] );
					rez['content' + margins[m]] = nelson_customizer_prepare_css_value( vars['page'] - vars['gap'] - vars['sidebar'] + margins[m] );
				}
			}
		}
		return rez;
	...
	// Add ed to css value
	function nelson_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function nelson_customizer_prepare_html_value(val) {
