...
?>>
	...
	if ($args['featured'] != 'none') {
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			...
			?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'nelson'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			echo !empty($link) 
				? '</a>' 
				: '</span>';
		} else {
	...
	<div class="sc_services_item_info">
		...
		<?php if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) { ?>
			<div class="sc_services_item_content"><?php
            $columns = (isset($args['columns']) && !empty($args['columns'])) ? $args['columns'] : 0;

            if( $columns > 3 ) {
                    echo wp_trim_words( $trim_excerpt, '8', '...' );
                } else {
                    echo wp_trim_words( $trim_excerpt, '20', '...' );
                }
            ?></div><?php
