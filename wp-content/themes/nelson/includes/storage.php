...
if ( ! function_exists( 'nelson_storage_get_array' ) ) {
	function nelson_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $NELSON_STORAGE;
		if ( empty( $key2 ) ) {
			return ! empty( $var_name ) && ! empty( $key ) && isset( $NELSON_STORAGE[ $var_name ][ $key ] ) ? $NELSON_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && ! empty( $key ) && isset( $NELSON_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $NELSON_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}
