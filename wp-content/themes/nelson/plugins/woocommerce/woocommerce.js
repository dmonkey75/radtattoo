	...
	jQuery( document ).on(
		...
		'action.ready_nelson', function() {
			...
			jQuery( '.woocommerce,.woocommerce-page' ).on(
				'click', '.nelson_shop_mode_buttons a', function(e) {
					var mode = jQuery( this ).hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
					nelson_set_cookie( 'nelson_shop_mode', mode, 365 );
			...
			if (jQuery( '.woocommerce div.quantity .q_inc,.woocommerce-page div.quantity .q_inc' ).length === 0) {
				...
				jQuery( '.woocommerce div.quantity,.woocommerce-page div.quantity' ).on(
					'click', '>span', function(e) {
						nelson_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					}
				);
			...
			function nelson_woocomerce_inc_dec_click(button) {
			...
			jQuery( document.body ).on(
				'updated_wc_div', function() {
					...
					if (jQuery( 'table.cart' ).length > 0) {
						nelson_woocommerce_update_cart( 'update' );
					}
				}
			);
			...
			if (jQuery( 'table.cart' ).length > 0) {
				nelson_woocommerce_update_cart( 'init' );
			}
			...
			jQuery( window ).resize(
				function() {
					"use strict";
					if (jQuery( 'table.cart' ).length > 0) {
						nelson_woocommerce_update_cart( 'resize' );
					}
				}
			);

			// Update cart display
			function nelson_woocommerce_update_cart(status){
				"use strict";
				setTimeout(
					function() {
						...
						if (NELSON_STORAGE['mobile_layout_width'] < w && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
							var tbl = jQuery( 'table.cart' );
							if ( tbl.length > 0 ) {
