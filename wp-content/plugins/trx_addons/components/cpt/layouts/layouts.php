...
if ( !function_exists( 'trx_addons_cpt_layouts_show_layout' ) ) {
	add_action( 'trx_addons_action_show_layout', 'trx_addons_cpt_layouts_show_layout', 10, 2 );
	function trx_addons_cpt_layouts_show_layout($id, $post_id=0, $echo=true) {
		static $layouts = array();
		$content = '';
		if ( (int) $id > 0 ) {
			if ( isset( $layouts[ "{$id}_{$post_id}" ] ) ) {
				$content = $layouts[ "{$id}_{$post_id}" ];
			} else {
				$layout = get_post($id);
				if (!empty($layout)) {
					...
					// Save content to future
					$layouts[ "{$id}_{$post_id}" ] = $content;
					// Restore postdata
