...
$map_type = function_exists('trx_addons_sc_googlemap') && ( trx_addons_get_option('api_google') != '' || ( !empty($args['count']) && $args['count'] == 1 ) )
				? 'google'
	...
	if ($query->found_posts > 0) {
		...
		?><div<?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> class="sc_properties sc_properties_<?php 
			...
			if ( $map_type == 'google' ) {
				...
				trx_addons_show_layout( trx_addons_sc_yandexmap($map_args) );
