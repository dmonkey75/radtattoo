...
if ( !function_exists( 'trx_addons_cpt_team_related_posts_show' ) ) {
	...
	function trx_addons_cpt_team_related_posts_show( $show ) {
		if (!$show && is_single() && get_post_type() == TRX_ADDONS_CPT_TEAM_PT) {
		...
		if ($mode == 'team.single') {
			...
			trx_addons_get_template_part('templates/tpl.posts-related.php',
												...
												apply_filters('trx_addons_filter_args_related', array(
																	...
																	'title' => sprintf( __('%s posts', 'trx_addons'), get_the_title() )
																	)
															)
											);
		}
...
if ( !function_exists( 'trx_addons_cpt_team_contact_form_after_article' ) ) {
	add_action('trx_addons_action_after_article', 'trx_addons_cpt_team_contact_form_after_article', 15, 1);
			...
			if ( !empty($form_id) && !trx_addons_is_off($form_id) ) {
				?><section class="page_contact_form team_page_form">
					<h3 class="section_title page_contact_form_title"><?php
						echo esc_html( apply_filters( 'trx_addons_filter_team_posts_title', __('Contact with me', 'trx_addons') ) );
					?></h3><?php
