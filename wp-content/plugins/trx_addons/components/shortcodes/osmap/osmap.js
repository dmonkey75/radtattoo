		...
		if (sc_osmap.length > 0) {
			sc_osmap.each(function () {
				...
				var map 		= jQuery(this).addClass('inited'),
					...
					map_style	= map.data('style') ? (map.data('style')=='default' ? 'basic' : map.data('style')) : 'streets',
...
//		};
	...
	function trx_addons_sc_osmap_build(id) {
		...
		// Prepare maps bounds
		osmap.maps[id].fit_to_bounds = ( osmap.maps[id].opt['center'].length > 0 || osmap.maps[id].markers.length > 1) && osmap.maps[id].opt['zoom'] < 1;
	...
	function trx_addons_sc_osmap_add_markers(id) {
			...
			if ( ! osmap.maps[id].markers[i].latlng ) {
				...
				osmap.geocoder.geocode( osmap.maps[id].markers[i].address, function(results) {
					if ( results[0] && results[0].center ) {
						try {
							osmap.maps[id].markers[osmap.maps[id].geocoder_request].latlng = [ results[0].center.lat, results[0].center.lng ];
							setTimeout(function() { 
								trx_addons_sc_osmap_add_markers(id);
								}, 1);
						} catch(e) {
		...
		if (osmap.maps[id].markers.length > 0 && osmap.maps[id].markers_inited == osmap.maps[id].markers.length) {
			...
			if (osmap.maps[id].fit_to_bounds) {
				...
				osmap.maps[id].map.setView(osmap.maps[id].opt['center'].length > 0 ? osmap.maps[id].opt['center'] : osmap.maps[id].markers[0].latlng, osmap.maps[id].opt['zoom'] > 0 ? osmap.maps[id].opt['zoom'] : 13);
			}
