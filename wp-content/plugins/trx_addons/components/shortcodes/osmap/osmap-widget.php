<?php
/**
 * Shortcode: OpenStreet Map (Widget)
 *
 * @package WordPress
...
if ( ! class_exists('TRX_Addons_SOW_Widget') ) {
	...
	class TRX_Addons_SOW_Widget_OpenStreetMap extends TRX_Addons_Widget {
	
		function __construct() {
			$widget_ops = array('classname' => 'widget_osmap', 'description' => esc_html__('Show OpenStreet map with specified address', 'trx_addons'));
			parent::__construct( 'trx_addons_sow_widget_osmap', esc_html__('ThemeREX OpenStreet map', 'trx_addons'), $widget_ops );
		}
		...
		function widget($args, $instance) {
			...
			if (!empty($instance['marker_address']) || !empty($instance['marker_latlng']))
				$output = trx_addons_sc_osmap(apply_filters('trx_addons_filter_widget_args',
						array_merge($instance, array(
							...
							$instance, 'trx_addons_sow_widget_osmap')
						);
		...
		function update($new_instance, $instance) {
			...
			return apply_filters('trx_addons_filter_widget_args_update', $instance, $new_instance, 'trx_addons_sow_widget_osmap');
		}
		...
		function form($instance) {
			...
			$instance = wp_parse_args( (array) $instance, apply_filters('trx_addons_filter_widget_args_default', array(
				...
				"class" => "",
				"css" => ""
				), 'trx_addons_sow_widget_osmap')
			);
		
			do_action('trx_addons_action_before_widget_fields', $instance, 'trx_addons_sow_widget_osmap', $this);
			...
			do_action('trx_addons_action_after_widget_title', $instance, 'trx_addons_sow_widget_osmap', $this);
			...
			$this->show_field(array('name' => 'style',
									...
									'options' => trx_addons_get_list_sc_osmap_styles(),
									'type' => 'select'));
			...
			do_action('trx_addons_action_after_widget_fields', $instance, 'trx_addons_sow_widget_osmap', $this);
		}
	}

	// Load widget
	if (!function_exists('trx_addons_sow_widget_osmap_load')) {
		add_action( 'widgets_init', 'trx_addons_sow_widget_osmap_load' );
		function trx_addons_sow_widget_osmap_load() {
			register_widget('TRX_Addons_SOW_Widget_OpenStreetMap');
		}
	}
}
