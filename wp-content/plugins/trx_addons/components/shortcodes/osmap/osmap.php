...
if ( !function_exists( 'trx_addons_sc_osmap_load_scripts_front' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_sc_osmap_load_scripts_front');
	function trx_addons_sc_osmap_load_scripts_front() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))){
			wp_enqueue_style( 'trx_addons-sc_osmap', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.css'), array(), null );
		}
	}
}

// Load responsive styles for the frontend
if ( !function_exists( 'trx_addons_sc_osmap_load_responsive_styles' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_sc_osmap_load_responsive_styles', 2000);
	function trx_addons_sc_osmap_load_responsive_styles() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-sc_osmap-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.responsive.css'), array(), null );
		}
	}
}
...
if ( !function_exists( 'trx_addons_sc_osmap_merge_styles' ) ) {
	...
	function trx_addons_sc_osmap_merge_styles($list) {
		$list[] = TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.css';
		return $list;
	}
}
...
if ( !function_exists( 'trx_addons_sc_osmap_merge_styles_responsive' ) ) {
	...
	function trx_addons_sc_osmap_merge_styles_responsive($list) {
		$list[] = TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.responsive.css';
		return $list;
	}
}
...
if ( !function_exists( 'trx_addons_sc_osmap_merge_scripts' ) ) {
	...
	function trx_addons_sc_osmap_merge_scripts($list) {
		$list[] = TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.js';
		return $list;
	}
}
...
if ( !function_exists( 'trx_addons_sc_osmap_load_scripts' ) ) {
	...
	function trx_addons_sc_osmap_load_scripts($editor='') {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))){
			wp_enqueue_style( 'trx_addons-sc_osmap', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.css'), array(), null );
		}
		...
		if (trx_addons_is_on(trx_addons_get_option('debug_mode')) && $editor!='gutenberg' && (!function_exists('trx_addons_gutenberg_is_preview') || !trx_addons_gutenberg_is_preview())) {
			wp_enqueue_script( 'trx_addons-sc_osmap', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'osmap/osmap.js'), array('jquery'), null, true );
		}
	}
}
...
if ( !function_exists( 'trx_addons_sc_osmap' ) ) {
	function trx_addons_sc_osmap($atts, $content=null){	
		...
		if ( (is_array($atts['markers']) && count($atts['markers']) > 0) || !empty($atts['address'])) {
			...
			if (empty($atts['style'])) $atts['style'] = 'basic';
