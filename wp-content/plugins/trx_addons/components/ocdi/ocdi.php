...
if (!function_exists('trx_addons_ocdi_options')){
	function trx_addons_ocdi_options($name='') {
		...
		elseif ( isset($ocdi_options[$name]) )
			...
			die(__("Undefined OCDI option '{$name}'", 'trx_addons'));
	}
}
	...
	function trx_addons_ocdi_export_options($options, $list) { 
		...
		foreach($options as $option_name){
			if (strpos($option_name, '%') !== false) {
				...
				if ($option_name == 'theme_mods_' . get_option( 'stylesheet' )){
...
if (!function_exists('trx_addons_ocdi_after_import')) {
	...
	function trx_addons_ocdi_after_import(){
		...
		if(!empty($import_elements)){
			...
			$keys = array('trx_addons_options', 'trx_addons_edd_demo_url',  get_option( 'stylesheet' ) . '_options', '_menu_item_url', '_wpb_shortcodes_custom_css', 'panels_data'); 	
	...
	function trx_addons_ocdi_replace_site_url($str, $old_url = '') {
		...
		return trx_addons_str_replace(
			array(
				...
				addslashes(trx_addons_remove_protocol($old_url)),
				trx_addons_remove_protocol($old_url, true),
				addslashes(trx_addons_remove_protocol($old_url, true))
			),
			array(
				$site_url,
				addslashes($site_url),
				trx_addons_remove_protocol($site_url),
				addslashes(trx_addons_remove_protocol($site_url)),
				trx_addons_remove_protocol($site_url, true),
				addslashes(trx_addons_remove_protocol($site_url, true))
			),
			$str
		);
	}
...
if (!function_exists('trx_addons_ocdi_export_site_data')) {
	...
	function trx_addons_ocdi_export_site_data($output){		
		...
		$options = array('blogname', 'blogdescription', 'posts_per_page', 'page_on_front', 'show_on_front', 'page_for_posts', 'theme_mods_' . get_option( 'stylesheet' ), 'sidebars_widgets', 'trx_addons_widgets_areas', 'trx_addons_options', 'widget_%' );
