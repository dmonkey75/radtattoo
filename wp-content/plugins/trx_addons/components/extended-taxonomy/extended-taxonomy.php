...
if ( !function_exists( 'trx_addons_extended_taxonomy_load_scripts_front' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_extended_taxonomy_load_scripts_front');
...
if ( !function_exists( 'trx_addons_extended_taxonomy_merge_styles' ) ) {
	...
	function trx_addons_extended_taxonomy_merge_styles($list) {
		if (trx_addons_extended_taxonomy_enable()) {
			$list[] = TRX_ADDONS_EXTENDED_TAXONOMY . 'css/extended-taxonomy.css';
		}
		return $list;
	}
}
...
if (!function_exists('trx_addons_get_term_image')) {
	function trx_addons_get_term_image($term_id=0, $taxonomy='', $check_parents=false) {
		$image = trx_addons_get_term_meta( array( 'term_id' => $term_id, 'taxonomy' => $taxonomy, 'key' => TRX_ADDONS_EXTENDED_TAXONOMY_META_PREFIX . 'image_large', 'check_parents' => $check_parents ) );
		if (empty($image)) {
			$image_id = trx_addons_get_term_meta( array( 'term_id' => $term_id, 'taxonomy' => '', 'key' => 'thumbnail_id', 'check_parents' => $check_parents ) );
			if (!empty($image_id)) {
...
if (!function_exists('trx_addons_get_term_image_small')) {
	function trx_addons_get_term_image_small($term_id=0, $taxonomy='', $check_parents=false) {
		$image = trx_addons_get_term_meta( array( 'term_id' => $term_id, 'taxonomy' => $taxonomy, 'key' => TRX_ADDONS_EXTENDED_TAXONOMY_META_PREFIX . 'image_small', 'check_parents' => $check_parents ) );
		return trx_addons_remove_protocol($image);
	}
}
...
if (!function_exists('trx_addons_get_term_icon')) {
	function trx_addons_get_term_icon($term_id=0, $taxonomy='', $check_parents=false) {
...
if (!function_exists('trx_addons_get_term_color')) {
	function trx_addons_get_term_color($term_id=0, $taxonomy='', $check_parents=false) {
		return trx_addons_get_term_meta( array( 'term_id' => $term_id, 'taxonomy' => $taxonomy, 'key' => TRX_ADDONS_EXTENDED_TAXONOMY_META_PREFIX . 'color', 'check_parents' => $check_parents ) );
	}
}
...
if (!function_exists('trx_addons_extended_taxonomy_show_fields')) {
	...
	function trx_addons_extended_taxonomy_show_fields ($term = false) {
		...
		if (in_array('image_large', $attrs)) {
			...
			?><input id="trx_addons_taxonomy_image_large" class="trx_addons_image_selector_field"
		...
		if (in_array('image_small', $attrs)) {
			...
			?><input id="trx_addons_taxonomy_image_small" class="trx_addons_thumb_selector_field"
