...
if (!function_exists('trx_addons_debug_calls_stack_screen')) {
	function trx_addons_debug_calls_stack_screen($depth=-1) {
		$s = debug_backtrace();
		array_shift($s);
		if ($depth > 0) $s = array_slice($s, 0, $depth);
...
if (!function_exists('trx_addons_debug_calls_stack_file')) {
	function trx_addons_debug_calls_stack_file($depth=-1) {
		$s = debug_backtrace();
		array_shift($s);
		if ($depth > 0) $s = array_slice($s, 0, $depth);
...
if (!function_exists('trx_addons_debug_dump_wp')) {
	function trx_addons_debug_dump_wp($query=null) {
		...
		echo "<tt>"
			...
			."<br>tax=".is_tax().($query ? "  query=".esc_html($query->is_tax()) : '')
