	...
	function trx_addons_get_video_layout($args=array()) {
		...
		if (!empty($args['cover'])) {
			$args['cover'] = trx_addons_get_attachment_url($args['cover'], 
										apply_filters('trx_addons_filter_video_cover_thumb_size', trx_addons_get_thumb_size('masonry-big')));
			if (!empty($args['cover'])) {
						...
						. '<div id="'.esc_attr($args['id']).'_popup_player"'
							...
							. '>'
								. '<div class="video_embed video_frame">'
									. str_replace(array('wp-video', 'src='), array('trx_addons_video', 'data-src='), $args['embed'])
								. '</div>'
							. '</div>'
						. '</div>';
		}
...
if (!function_exists('trx_addons_get_attachment_url')) {
	function trx_addons_get_attachment_url($image_id, $size='full') {
		 if ( is_array( $image_id ) ) {
			  $image_id = ! empty( $image_id['id'] ) ? $image_id['id'] : ( ! empty( $image_id['url'] ) ? $image_id['url'] : '' );
		 }
		 if ( is_numeric( $image_id ) && (int) $image_id > 0) {
		...
		if (($pos = strpos($video, '<video'))!==false) {
			...
			$video = preg_replace_callback(
				...
				function($matches) {
					...
					return str_replace( '<iframe ', '<iframe allow="autoplay" ', $matches[1] ) . $matches[2] . $matches[3];
				},
				$video);
		}
		return $video;
