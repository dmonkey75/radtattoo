													...
													. '>'
														...
														. ( !empty($v['override']['desc']) || !empty($v['desc'])
															? ( '<div class="trx_addons_options_group_description">'
																...
																. '</div><!-- /.trx_addons_options_group_description -->'
																)
															: ''
															);
		...
		if (count($tabs_content) > 0) {
			...
			<div class="trx_addons_tabs <?php echo count($tabs_titles) > 1 ? 'with_tabs' : 'no_tabs'; ?>">
				<?php if (count($tabs_titles) > 1) { ?>
					<ul><?php
						foreach ($tabs_titles as $k=>$v) {
							if ( empty($tabs_content[$k]) || $tabs_empty[$k] ) continue;
				...
				foreach ($tabs_content as $k=>$v) {
					if ( empty($v) || $tabs_empty[$k] ) continue;
					?>
			...
			case 'osmap':
			    ...
			    if ($field['type'] == 'googlemap') {
					...
					if (trx_addons_check_option('api_google_load') && trx_addons_is_on(trx_addons_get_option('api_google_load')) && trx_addons_get_option('api_google') != '') {
						$map_type = 'googlemap';
						trx_addons_enqueue_googlemap();
					} else if (trx_addons_check_option('api_yandex_load') && trx_addons_is_on(trx_addons_get_option('api_yandex_load'))) {
						$map_type = 'yandexmap';
						trx_addons_enqueue_yandexmap();
